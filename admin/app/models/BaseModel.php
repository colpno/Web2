...
class BaseModel extends Database
{
    ...
    protected function postMethod($tableName, $data = [], $maxID = null)
    {
        ...
        */
        ...
        for ($i = 0; $i < sizeof($arrKeys); $i++) {
            $this->checkExistence($tableName, $arrKeys[$i], $arrValues[$i]);
        }
    ...
    {
        $sql = "UPDATE $tableName SET tinhTrang = " . $data['tinhTrang'] . " WHERE maHD = " . $data['maHD'];
        if ($this->conn->query($sql)) {
            return;
        } else {
    ...
    private function checkExistence($tableName, $col, $value)
    {
        $sql = "SELECT EXISTS(SELECT * FROM $tableName WHERE $col = '$value') as exist ";
        $query = $this->conn->query($sql);
        $exist = null;
        while ($row = mysqli_fetch_assoc($query)) {
            $exist = $row['exist'];
